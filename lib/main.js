...
class RecentList {
  constructor() {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      confirmAction: "recent-list:open-in-new-window",
      additionalActionCommands: ["application:clear-project-history"],
      ...
      actionsFilter: ({ name }) =>
        name !== "application:clear-project-history" || lumine.history.getProjects().length > 0,
    });
